...
export default function usuario({ usuarios, roles }) {
  ...
  const { data } = useSWR(
    [
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/users?populate[0]=role&pagination[page]=${pageIndex}&pagination[pageSize]=5`,
      {
        method: "GET",
        headers: {
...
export async function getServerSideProps({ req, params }) {
  ...
  const usuariosResponse = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/users?populate[0]=role&pagination[page]=1&pagination[pageSize]=5`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
