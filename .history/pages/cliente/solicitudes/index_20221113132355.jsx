...
export default function solicitudes({ solicitudes, locales }) {
  ...
  const { data } = useSWR(
    [
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/solicitudes?populate[0]=local&populate[1]=elaboradopor&populate[2]=demanda_recursos&populate[3]=ordenes_de_trabajos&populate[4]=local.arearesponsable&populate[5]=local.arearesponsable.unidadorganizativa&populate[6]=local.inmueble&filters[local][arearesponsable][responsable][id][$eq]=${iduser}&pagination[page]=${pageIndex}&pagination[pageSize]=5`,
      {
        method: "GET",
        headers: {
...
export async function getServerSideProps({ req, params }) {
  ...
  const solicitudesResponse = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/solicitudes?populate[0]=local&populate[1]=elaboradopor&populate[2]=demanda_recursos&populate[3]=ordenes_de_trabajos&populate[4]=local.arearesponsable&populate[5]=local.arearesponsable.unidadorganizativa&populate[6]=local.inmueble&filters[local][arearesponsable][responsable][id][$eq]=${iduser}&pagination[page]=1&pagination[pageSize]=5`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
