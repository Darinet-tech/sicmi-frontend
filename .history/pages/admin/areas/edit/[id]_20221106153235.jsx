...
export default function editPage({ responsables }) {
  ...
  const handleChange = (e) => {
    setUnidadorganizativa({ ...uo, [e.target.name]: e.target.value });
  };

  const handleSubmit = async () => {
    try {
      const responseData = await fetcher(
        `${process.env.NEXT_PUBLIC_STRAPI_URL}/unidadorganizativas/${uo.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const loaduo = async (pid) => {
    try {
      const uo_loaded = await fetcher(
        `${process.env.NEXT_PUBLIC_STRAPI_URL}/unidadorganizativas/${pid}?populate[0]=responsable`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      setArea({
        id: uo_loaded.data.id,
        nombre: uo_loaded.data.attributes.nombre,
        responsable: uo_loaded.data.attributes.responsable.data
          ? uo_loaded.data.attributes.responsable.data.id
          : "",
      });
    } catch (error) {
