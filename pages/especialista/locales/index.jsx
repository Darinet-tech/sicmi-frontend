...
import AddLocal from "../../../components/local/AddLocal";
import TableLocales from "../../../components/local/TableLocales";

export default function locales({ locales, areas, inmuebles }) {
  ...
  const { data } = useSWR(
    [
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/locals?populate[0]=arearesponsable&populate[1]=inmueble&filters[inmueble][unidadorganizativa][id][$eq]=${uo}&pagination[page]=${pageIndex}&pagination[pageSize]=5`,
      {
        method: "GET",
        headers: {
      ...
      <LayoutEspecialista>
        <>
          {uo ? (
            <Grid>
              <Row>
                <AddLocal areas={areas} inmuebles={inmuebles} />

                <Button
                  ghost
                  auto
                  onClick={() => router.push("/especialista/locales/all")}
                >
                  Mostrar todos
                </Button>
              </Row>
              <Row>
                {locales.data && locales.data.length > 0 ? (
                  ...
                  <h3>No existen Locales registrados</h3>
                )}
              </Row>
            </Grid>
          ) : (
...
export async function getServerSideProps({ req, params }) {
  ...
  const localesResponse = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/locals?populate[0]=arearesponsable&populate[1]=inmueble&filters[inmueble][unidadorganizativa][id][$eq]=${uo}&pagination[page]=1&pagination[pageSize]=5`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
